...
import { getOrCreateCustomer, buildMemoryContext, buildConversationHistory } from './memory.js';
...
export async function handleProcessCall({
  ...
  const { customer, isReturning } = await getOrCreateCustomer(phone_number, call_sid);
  const memoryContext = await buildMemoryContext(customer.id);
  const conversationHistory = await buildConversationHistory(call_sid);
  const openaiResult = await processCall(transcript, business, memoryContext, conversationHistory);
  ...
  try {
    ...
    if (String(err.message).includes('audio_base64')) {
      const withoutAudio = { ...callPayload };
      delete withoutAudio.audio_base64;
